...
double stars_distance(int i, int j) {
	return sqrt(pow(stars[i].x - stars[j].x, 2) + pow(stars[i].y - stars[j].y, 2));
}
...
void update_velocities(const double dt)
{
	...
	for (i = 0; i < nstars; i++) {
		...
		for (j = 0; j < nstars; j++) {
			if (i != j) {
				double distance = stars_distance(i, j);
...
void update_positions(const double dt)
{
	...
	for (i = 0; i < nstars; i++) {
		if (stars[i].m != 0.0) {
			...
			for (j = 0; j < nstars; j++) {
				if (i != j && stars[j].m != 0.0 && stars_distance(i, j) < 1.0) {
